...
export function PurchaseForm({
  ...
  function unitOptionsFor(itemId: number | null): Option[] {
    ...
    if (item.purchaseUnitId) opts.push({ id: item.purchaseUnitId, label: `${item.purchaseUnitLabel} (×${item.purchaseUnitFactor})` });
    return opts;
  }
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            ...
            <Select name="supplierId" required items={suppliers.map((s) => ({ value: String(s.id), label: s.label }))}>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="paidAmount">المدفوع الآن (اختياري)</Label>

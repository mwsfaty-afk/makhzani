...
export default async function SalesReportPage({
  ...
  const postedTotal = sales.filter((s) => s.status === "POSTED").reduce((sum, s) => sum + Number(s.grandTotal), 0);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" render={<Link href={`/api/reports/sales/csv?${qs}`} />}>
          <Download />
          تصدير CSV
        </Button>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">الفواتير ({sales.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {sales.length === 0 ? (
